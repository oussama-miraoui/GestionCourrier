        ...
        <table class="table table-hover table-bordered table-responsive" id="myTable">
            ...
            foreach ($rows as $k ) {
                ...
                <tr class="text-center bg-light">
                    ...
                    <td><?php echo $k['motscle'] ?></td>
                    <td><?php echo $k['observation'] ?></td>
                    <td style="font-size: 20px;width: 6%;">
                        <a class="modifC btn text-primary modif" id="modif" onclick="enableButton(); topFunction();" style="cursor: pointer;" ><i class="fa fa-edit" style="font-size:22px"></i></a>
                        <a href="action.php?delete=<?php echo $k['id'] ?>" onclick="return confirm('Vous-êtes sûr?');" class="btn text-danger" ><i class="fa fa-trash" style="font-size:22px"></i></a>
                    </td>
                </tr>
                <?php
            }
...
$(function () {
    $('input[type=file]').change(function () {
        var val = $(this).val().toLowerCase(),
            regex = new RegExp("(.*?)\.(docx|doc|pdf|jpg|png|txt)$");

        if (!(regex.test(val))) {
            $(this).val('');
            alert('Veuillez selectionner un fichier des formats suivantes : docx, doc, pdf, jpg, png, txt');
        }
    });
});
